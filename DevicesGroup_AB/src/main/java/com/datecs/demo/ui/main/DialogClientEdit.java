...
import com.datecs.demo.DialogClientEditBinding;
...
public class DialogClientEdit extends Dialog implements View.OnClickListener {
    private DialogClientEditBinding binder;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binder.btnReadItem.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    cmdClients.ClientInfoModel readedItem = myClients.findClientByEIK(binder.edCliEIK.getText().toString());
        ...
        binder.btnItemSaveToDevice.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    cmdClients.ClientInfoModel clintData = updateItem();
                    if (validateClientData(clintData)) {
                        sendBroadCastItemIsSet(clintData);
                        dismiss();
                    }
                } catch (Exception e) {
                    postToast(e.getMessage());
        ...
        binder.btnItemDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    dialog.setMessage(a.getString(R.string.msg_remove_client) + binder.edCliEIK.getText().toString());
                    dialog.setPositiveButton(R.string.yes, new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            sendBroadCastItemIsDeleted(binder.edCliEIK.getText().toString());
                            dialog.dismiss();
                            dismiss();
                        }
                    });
    ...
    private void sendBroadCastItemIsDeleted(String itemEIK) {
        Intent intent = new Intent("deleteClient");
        intent.putExtra("itemEIK", itemEIK);
        LocalBroadcastManager.getInstance(a.getBaseContext()).sendBroadcast(intent);
    }
    ...
    private void updateView(cmdClients.ClientInfoModel item) {
        ...
        binder.edCliEIK.setText(String.valueOf(item.getEIK()));
        binder.edCliName.setText(item.getName());
        binder.edCliVAT.setText(item.getVATN());
        binder.spTypeEIK.setSelection(item.getTypeTAXN().ordinal());
    ...
    private cmdClients.ClientInfoModel updateItem() {
        ...
        cmdClients.ClientInfoModel res = new cmdClients.ClientInfoModel(
                ...
                binder.edCliEIK.getText().toString(),
                dd.fromOrdinal((int) binder.spTypeEIK.getSelectedItemId()),
